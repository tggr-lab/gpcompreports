...
- **Stage explicit paths. Never `git add -A` or `git add -A <dir>`.** This repository has several untracked build-output directories sitting next to the source (`output_par2_preview/`, `output_tm_preview/`, and others). A broad add sweeps them into the commit: it already happened once on this branch, putting 23 files of stale build output into history before it was caught and reverted. Name the files you changed. If a commit step in a task below still shows `git add -A`, replace it with explicit paths.
