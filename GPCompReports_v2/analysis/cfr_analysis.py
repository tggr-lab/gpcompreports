...
def run_cfr_analysis(store):
    ...
    results['fig_pca_scatter'] = build_pca_scatter(store, cfr_table)
...
def build_pca_scatter(store, cfr_table, top_n=30):
    """Receptor PCA in CFR-aligned ΔRRCS space.

    Feature vector per receptor: signed ΔRRCS at each of the top-N CFR
    generic-numbered positions (max-magnitude contact incident on the
    residue at that GN; zero if the receptor has no detected contact at
    that position). PCA reduces to 2D; the scatter is colored by
    receptor family by default and a Plotly button menu toggles to
    ligand-type colors.
    """
    ...
    top_gns = cfr_table.head(top_n)['generic_number'].tolist()
    info_df = store.get_all_info_df().set_index('gpcr_id')
    ...
    for gid in store.gpcr_ids:
        ...
        pos_to_gn = {}
        for pos, info in annot_map.items():
            gn = (info.get('display_number') or info.get('generic_number') or '').strip()
            if gn:
                pos_to_gn[pos] = gn

        # Per-GN signed ΔRRCS = the signed delta of the largest-magnitude
        # contact incident on any residue mapped to that GN.
        gn_signed = {}
        for _, row in delta_df.iterrows():
            d = float(row['delta_rrcs'])
            for col in ('res1', 'res2'):
                pos = int(row[col]) if pd.notna(row[col]) else None
                if pos is None:
                    continue
                gn = pos_to_gn.get(pos)
                if gn and (gn not in gn_signed or abs(d) > abs(gn_signed[gn])):
                    gn_signed[gn] = d

        feature_rows.append([gn_signed.get(gn, 0.0) for gn in top_gns])
        gids_kept.append(gid)
    ...
    df = info_df.loc[gids_kept].reset_index()
    df['pc1'] = coords[:, 0]
    df['pc2'] = coords[:, 1]
    df['receptor_family'] = df['receptor_family'].fillna('Unknown')
    df['ligand_type'] = df['ligand_type'].fillna('Unknown')
    df['gene_name'] = df['gene_name'].fillna('')

    family_palette = px.colors.qualitative.Set2 + px.colors.qualitative.Set3
    ligand_palette = px.colors.qualitative.Pastel + px.colors.qualitative.Vivid

    def _trace(group, name, color):
        return go.Scatter(
            x=group['pc1'].tolist(),
            y=group['pc2'].tolist(),
            mode='markers',
            ...
            marker=dict(size=9, color=color, opacity=0.78,
                        line=dict(width=0.5, color='rgba(20,20,20,0.4)')),
            customdata=group[['gpcr_id', 'uniprot_name', 'gene_name',
                              'receptor_family', 'ligand_type']].values,
            hovertemplate=(
                '<b>%{customdata[1]}</b><br>'
                'Gene: %{customdata[2]}<br>'
                'Family: %{customdata[3]}<br>'
                'Ligand type: %{customdata[4]}<br>'
                'PC1: %{x:.2f}, PC2: %{y:.2f}<extra></extra>'
            ),
        )

    family_traces = []
    for i, (fam, grp) in enumerate(df.groupby('receptor_family')):
        family_traces.append(_trace(grp, fam, family_palette[i % len(family_palette)]))

    ligand_traces = []
    for i, (lig, grp) in enumerate(df.groupby('ligand_type')):
        t = _trace(grp, lig, ligand_palette[i % len(ligand_palette)])
        t.visible = False
        ligand_traces.append(t)

    fig = go.Figure(data=family_traces + ligand_traces)

    n_fam = len(family_traces)
    n_lig = len(ligand_traces)
    fig.update_layout(
        height=620,
        title=(
            f'Receptor PCA in CFR-aligned ΔRRCS space '
            f'(PC1 {var[0]:.0%}, PC2 {var[1]:.0%}, top {top_n} CFR positions)'
        ),
        xaxis_title=f'PC1 ({var[0]:.1%})',
        yaxis_title=f'PC2 ({var[1]:.1%})',
        legend_title_text='Receptor family',
        hovermode='closest',
        ...
        updatemenus=[dict(
            ...
            buttons=[
                dict(
                    ...
                    method='update',
                    args=[
                        {'visible': [True] * n_fam + [False] * n_lig},
                        {'legend.title.text': 'Receptor family'},
                    ],
                ),
                dict(
                    label='Color: ligand type',
                    method='update',
                    args=[
                        {'visible': [False] * n_fam + [True] * n_lig},
                        {'legend.title.text': 'Ligand type'},
                    ],
                ),
            ],
        )],
